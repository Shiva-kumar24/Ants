...
class Ant(Insect):
    ...
    def double(self):
        ...
        # BEGIN Problem 12
        if not getattr(self, 'has_doubled', False):
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not thrower_ant_place.is_hive and thrower_ant_place is not None:
            if count > self.upper_bound:
                return None
            elif count >= self.lower_bound and count <= self.upper_bound:
                if thrower_ant_place.bees:
                    return random_bee(thrower_ant_place.bees)
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True
    lower_bound = 5   # Change to True to view in the GUI
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if not self.place:
            total = total + self.damage
        for y in list(dic):
            Insect.reduce_health(y,total)
...
class QueenAnt(ThrowerAnt):
    ...
    def reduce_health(self, amount):
        """
        Reduce health by AMOUNT, and if the QueenAnt has no health remaining,
